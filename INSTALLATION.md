...
```powershell
# Auf <SQLInstance> als Admin ausführen:
sqlcmd -S <SQLInstance> -i "C:\Program Files\Tivoli\TSM\TDPSql\TDP-Backup-System\01_TDP_BackupTracking_Setup.sql"

# Verifikation:
sqlcmd -S <SQLInstance> -Q "SELECT COUNT(*) AS [Tables] FROM [master].[INFORMATION_SCHEMA].[TABLES] WHERE [TABLE_NAME] LIKE 'TDP_%' OR [TABLE_NAME] LIKE 'Backup%'"
```
...
```json
{
  "Configuration": {
    ...
    "AlertEmail": "backup-admin@example.com"    ← Anpassen!
  },
  "SelectedDatabases": [
    {
      "Name": "AdventureWorks",
...
```powershell
# Auf <SQLInstance> als Admin ausführen:
sqlcmd -S <SQLInstance> -i "C:\Program Files\Tivoli\TSM\TDPSql\TDP-Backup-System\02_Create_Agent_Job.sql"
```
...
# Direkter Test (SQL-Instance kommt aus TdpBackup.config, siehe Setup-TdpBackup.ps1)
.\Backup-TdpFull.ps1
...
# SQL-Logs
sqlcmd -S <SQLInstance> -Q "SELECT TOP 20 * FROM [master].[dbo].[BackupLog] ORDER BY [ExecutionDate] DESC"
```

### **Status-Report**

```powershell
sqlcmd -S <SQLInstance> -Q "EXEC [master].[dbo].[sp_GetBackupStatusReport]"
```
...
| SQL-Verbindung fehlgeschlagen | SQL Server offline oder wrong credentials | `sqlcmd -S <SQLInstance> -Q "SELECT @@VERSION"` testen |
