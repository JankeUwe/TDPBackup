...
$buttonRemoveDatabase.Add_Click({
    $selectedRows = @($dataGridDatabases.SelectedRows)
    if ($selectedRows.Count -eq 0) {
        [System.Windows.Forms.MessageBox]::Show("Bitte zuerst eine oder mehrere Zeilen auswählen.", "Hinweis",
            [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Information)
        return
    }
    foreach ($row in $selectedRows) {
        $dataGridDatabases.Rows.Remove($row)
    }
})
