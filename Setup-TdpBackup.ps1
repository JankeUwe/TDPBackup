...
#
# USAGE:
#   .\Setup-TdpBackup.ps1 -SQLInstance "<SQLInstance>" -TSMPassword "xxx"
...
function Save-Config {
    ...
    $secureTsmPassword = ConvertTo-SecureString -String $TSMPassword -AsPlainText -Force

    $config = @{
        SQLInstance = $SQLInstance
        TSMPassword = $secureTsmPassword
        SQLServer = $SQLServer
...
function Show-SetupWizard {
    ...
    $textSqlInstance.Text = $SQLInstance
